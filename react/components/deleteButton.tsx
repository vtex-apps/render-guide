...
import { CachedBookList, CachedTotal, Maybe } from '../utils/interfaces'
...
const isValidIndex = <T extends unknown>(array: Maybe<T[]>, index: Maybe<number>) => !!(Array.isArray(array) && index != null && -1 < index && index < array.length)
...
const update = (id: string): MutationUpdaterFn<any> => (cache, {data: {delete: status}}) => {
...
export const DeleteButton: React.SFC<{id: string}> = ({id}) => (
  ...
  {(deleteBook, {loading}) => loading
  ? <Spinner />
  : (
    <Button onClick={onClickDelete(id, deleteBook)}>
      Delete
    </Button>
  )}
  </Mutation>
